...
#' 
pops_multiple <- function(infected_file, host_file, total_plants_file, reproductive_rate = 3.0,
  ...
  series <- vector("list", runs)
  

  #for (seed in random_seed:(random_seed+runs-1)) {
  for (i in 1:runs) {
      seed = random_seed + i - 1
      data <- pops_model(random_seed = seed,
